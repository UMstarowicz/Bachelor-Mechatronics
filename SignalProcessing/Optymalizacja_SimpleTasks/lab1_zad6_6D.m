...
[F.Data(sample).Normal.Moments.M11,F.Data(sample).Normal.Area,F.Data(sample).Normal.Moments.M30,F.Data(sample).Normal.HuInvariants.I0,F.Data(sample).Normal.Moments.N11,F.Data(sample).Normal.HuInvariants.I1];
 TrainingDataClass1(:,sample) = ...
[T.Data(sample).Normal.Moments.M11,T.Data(sample).Normal.Area,T.Data(sample).Normal.Moments.M30,T.Data(sample).Normal.HuInvariants.I0,T.Data(sample).Normal.Moments.N11,T.Data(sample).Normal.HuInvariants.I1];
end
...
for k = 301:600
 ...
 F5 = F.Data(k).Normal.Moments.N11/Means(5);
 F6 = F.Data(k).Normal.HuInvariants.I1/Means(6); 
 
 if(ClassifierKNN_K([F1,F2,F3,F4,F5,F6],TrainingDataClass0,TrainingDataClass1) == 1) % Detected class "1" 
ErrorsF = ErrorsF + 1;
 ...
 F5 = T.Data(k).Normal.Moments.N11/Means(5);
 F6 = T.Data(k).Normal.HuInvariants.I1/Means(6); 

 if(ClassifierKNN_K([F1,F2,F3,F4,F5,F6],TrainingDataClass0,TrainingDataClass1) == 0) % Detected class "0 
ErrorsT = ErrorsT + 1;
