...
FunctionForOptimization2 = str2func('Parametry_W1_W2_AG_VA');
...
%          NewPopulation(k).Parameters = min(MaxRangeX(2),max(NewPopulation(k).Parameters,MaxRangeX(1)));
         ...
         PHistory(iter).Parameters = Population(Indices(1)).Parameters;
         val(1,iter) = FunctionForOptimization2(Population(Indices(1)).Parameters);
         val(2,iter) = iter;
